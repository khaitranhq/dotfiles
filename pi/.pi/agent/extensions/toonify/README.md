...
**Why:** TOON reduces token count by ~40% vs JSON while maintaining or improving LLM comprehension accuracy (76.4% vs 75.0% per benchmarks). Every tool result that returns JSON — `bash` (ls, find, grep), `read` (configs), MCP tools, custom tool output — gets automatically converted.
