...
import { interpolateEnvVars, interpolateEnvRecord, resolveBearerToken } from "./utils";

const ENV_KEY = "MCP_TEST_TOKEN";
const ENV_KEY2 = "MCP_TEST_SECRET";
...
describe("interpolateEnvVars", () => {
  ...
  it("replaces undefined env var with empty string ($env:VAR)", () => {
    // $env:VAR has no closing delimiter, so \w+ greedily consumes trailing word chars.
    // Use a non-word delimiter to separate the var name.
...
describe("resolveBearerToken", () => {
  ...
  it("returns interpolated bearerToken value", () => {
    const result = resolveBearerToken({
      bearerToken: `token-\${${ENV_KEY}}`,
    });
    expect(result).toBe(`token-${ENV_VALUE}`);
  });

  it("returns env var value for bearerTokenEnv", () => {
    const result = resolveBearerToken({
      bearerTokenEnv: ENV_KEY,
    });
    expect(result).toBe(ENV_VALUE);
  });

  it("returns undefined when bearerTokenEnv points to nonexistent env var", () => {
    const result = resolveBearerToken({
      bearerTokenEnv: "NONEXISTENT_VAR_XYZ",
    });
    expect(result).toBeUndefined();
  });

  it("returns undefined when neither bearerToken nor bearerTokenEnv is set", () => {
    const result = resolveBearerToken({});
    expect(result).toBeUndefined();
  });

  it("returns undefined when bearerToken is undefined and bearerTokenEnv is not set", () => {
    const result = resolveBearerToken({ bearerToken: undefined });
    expect(result).toBeUndefined();
  });

  it("prefers bearerToken over bearerTokenEnv when both are set", () => {
    setEnv("OTHER_TOKEN", "other-value");
    const result = resolveBearerToken({
      bearerToken: `direct-\${${ENV_KEY}}`,
      bearerTokenEnv: "OTHER_TOKEN",
    });
    expect(result).toBe(`direct-${ENV_VALUE}`);
  });

  it("interpolates bearerToken with $env:VAR pattern", () => {
    const result = resolveBearerToken({
      bearerToken: `$env:${ENV_KEY}`,
    });
    expect(result).toBe(ENV_VALUE);
  });
});
