...
export function renderCall(args: Record<string, unknown>, theme: any, _context: any) {
  if (args.chain && (args.chain as any[]).length > 0) {
    const chain = args.chain as any[];
    let text =
  ...
  if (args.tasks && (args.tasks as any[]).length > 0) {
    ...
    for (const t of tasks.slice(0, 3)) {
      const preview =
        (t.task as string).length > 40
          ...
          : (t.task as string);
      text += `\n  ${theme.fg("accent", t.agent as string)}${theme.fg("dim", ` ${preview}`)}`;
    }
