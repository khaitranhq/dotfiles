/**
 * Subagent Extension — Primary & Subagent support (opencode-compatible)
 *
 * Spawns a separate `pi` process for each subagent invocation with isolated
 * context. Primary agents can be switched via `/agent <name>`.
 *
 * Supports three subagent modes:
 *   - Single:  { agent: "name", task: "..." }
 *   - Parallel: { tasks: [{ agent: "name", task: "..." }, ...] }
 *   - Chain:    { chain:   [{ agent: "name", task: "... {previous} ..." }, ...] }
 *
 * Agent discovery locations (markdown files with YAML frontmatter):
 *   User-level:    ~/.pi/agent/agents/*.md
 *   Project-level: .pi/agents/*.md (travelled up from cwd)
 *
 * Frontmatter schema (opencode-compatible):
 *   name: agent-name          # required
 *   description: description  # required
 *   mode: primary|subagent    # default: subagent
 *   model: claude-sonnet-4-5  # optional model override
 *   tools: read,bash,edit     # optional tool list
 *   ---
...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params, signal, onUpdate, ctx) {
      ...
      // Confirm project-local agents interactively
      if (
        ...
        const projectAgentsRequested = Array.from(requestedAgentNames)
          .map((name) => subagents.find((a) => a.name === name))
    ...
    renderResult(result, { expanded }, theme, _context) {
      ...
      // ── Aggregate usage ──────────────────────────────────────────────
      const aggregateUsage = (rs: SingleResult[]) => {
        const total = {
          input: 0,
          output: 0,
          cacheRead: 0,
  ...
  // ── Command: /agent — switch or list primary agents ──────────────────
  ...
  pi.on("before_agent_start", async (event, ctx) => {
    if (!activePrimaryAgent) return;

    const discovery = discoverAgents(ctx.cwd, "both");
    const agent = discovery.primaryAgents.find((a) => a.name === activePrimaryAgent);
