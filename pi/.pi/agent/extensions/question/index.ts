...
const QuestionTypeSchema = StringEnum(["input", "select", "multi_select"] as const, {
  description:
...
export default function question(pi: ExtensionAPI) {
  pi.registerTool({
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      ...
      const questions: QuestionDefinition[] = params.questions.map((q, i) => {
      ...
      // ── Render the UI ───────────────────────────────────────────────
      const result = await ctx.ui.custom<QuestionResult>((tui, theme, _kb, done) => {
        // State
        let currentTab = 0;
        let optionIndex = 0;
        let inputMode = false;
        ...
        function render(width: number): string[] {
          ...
          if (currentTab === questions.length) {
            ...
            add("");
            if (isMulti) {
              add(theme.fg("dim", " Enter to submit • Tab to navigate • Esc to cancel"));
            } else {
            ...
            const existing = answers.get(q.id);
            if (existing && existing.values.length > 0) {
              add(`${theme.fg("success", "✓")} ${theme.fg("text", existing.values[0])}`);
              add("");
            }

            add(theme.fg("accent", ` ${theme.bold("Click Enter to type your answer")} ✎`));
            add("");
            if (isMulti) {
              add(theme.fg("dim", " Enter to type • Tab/←→ to navigate • Esc to cancel"));
            } else {
              add(theme.fg("dim", " Enter to type • Esc to cancel"));
            }
            ...
            for (let i = 0; i < opts.length; i++) {
              ...
              let indicator = "";
              if (isMultiSelect) {
                const sel = multiSelected.get(q.id) || new Set<string>();
                ...
                indicator = selected ? theme.fg("accent", ">") : " ";
              }

              const numPrefix = theme.fg(selected ? "accent" : "dim", `${i + 1}.`);

              // Mark "Type something" differently
              if (isOther) {
                if (selected) {
                  add(`${indicator} ${numPrefix} ${theme.fg("accent", `${opt.label} ✎`)}`);
                } else {
                  add(`${indicator} ${numPrefix} ${theme.fg("muted", opt.label)}`);
                }
              } else if (selected) {
                add(`${indicator} ${numPrefix} ${theme.fg("accent", opt.label)}`);
              } else {
                add(`${indicator} ${numPrefix} ${theme.fg("text", opt.label)}`);
              }
            ...
            if (isMultiSelect) {
              const sel = multiSelected.get(q.id) || new Set<string>();
              const checkedOpts = q.options.filter((o) => sel.has(o.value));
              const customCount = Array.from(sel).filter((k) => k.startsWith("__custom__")).length;

              if (checkedOpts.length > 0 || customCount > 0) {
                add("");
                const parts: string[] = [];
                for (const o of checkedOpts) {
                  parts.push(theme.fg("success", o.label));
                }
                for (let i = 0; i < customCount; i++) {
                  parts.push(theme.fg("muted", "(custom)"));
                }
            ...
            if (isMultiSelect) {
              if (isMulti) {
                add(theme.fg("dim", " Space/Enter to toggle • Tab/←→ to navigate • Esc to cancel"));
              } else {
                add(theme.fg("dim", " Space to toggle • Enter to submit • Esc to cancel"));
              }
            } else {
              if (isMulti) {
                add(
      ...
      if (result.cancelled) {
        return {
          content: [{ type: "text", text: "User cancelled the question(s)" }],
          details: result,
        };
      }
      ...
      const answerLines = result.answers.map((a) => {
        ...
        if (a.customValues.length > 0) {
          const selected = a.labels.filter((l) => !a.customValues.includes(l)).join(", ");
